...
namespace
{
	...
	IMPLEMENT_SIMPLE_AUTOMATION_TEST(
		FPhysAnimFirstPolicyObservationSourceExperimentContractTest,
		"PhysAnim.Bridge.FirstPolicyObservationSourceExperimentContract",
		EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

	bool FPhysAnimFirstPolicyObservationSourceExperimentContractTest::RunTest(const FString& Parameters)
	{
		auto MakeBodySamples = [](const double RootHeightM, const double Seed)
		{
			...
			{
				Samples[BodyIndex].Position = FVector(
					Seed + BodyIndex * 0.01,
					-Seed - BodyIndex * 0.02,
					BodyIndex == 0 ? RootHeightM : RootHeightM + BodyIndex * 0.005);
				Samples[BodyIndex].Rotation = FQuat::Identity;
				Samples[BodyIndex].LinearVelocity = FVector(Seed, BodyIndex, -Seed);
				Samples[BodyIndex].AngularVelocity = FVector(-Seed, BodyIndex * 0.5, Seed);
			}
			return Samples;
		};
		auto MakeGroundReference = [](const double BodyRootProtoZM, const double RootBoneWorldZCm)
		{
			...
			Values.StaticTraceImpactZCm = 20.0;
			Values.bCapsuleAvailable = true;
		...
		const FPhysAnimSelfObservationGroundReferenceValues PriorGround =
			MakeGroundReference(PriorBodySamples[0].Position.Z, 130.0);

		auto ExerciseMode = [&](const EPhysAnimFirstPolicyObservationSourceMode Mode)
		{
			const bool bUsesPriorBody = Mode != EPhysAnimFirstPolicyObservationSourceMode::LiveBodyLiveGround;
			const bool bUsesPriorGround = Mode == EPhysAnimFirstPolicyObservationSourceMode::PriorBodyPriorGround;
			const TArray<FPhysAnimBodySample>& ExpectedBodySamples = bUsesPriorBody
				? PriorBodySamples
				: LiveBodySamples;
			const FPhysAnimSelfObservationGroundReferenceValues LiveGround =
				MakeGroundReference(ExpectedBodySamples[0].Position.Z, 150.0);

			FPhysAnimFirstPolicyObservationSourceExperimentTrace Trace;
			FString Error;
			TestTrue(
				FString::Printf(TEXT("%s configuration is accepted"), GetFirstPolicyObservationSourceModeName(Mode)),
				Trace.Configure(bUsesPriorBody, Mode, Error));
			TArray<FPhysAnimBodySample> EffectiveBodySamples;
			TestTrue(
				FString::Printf(TEXT("%s body stage succeeds: %s"), GetFirstPolicyObservationSourceModeName(Mode), *Error),
				Trace.SelectBodySamples(PriorBodySamples, LiveBodySamples, EffectiveBodySamples, Error));
			TestEqual(TEXT("The body stage publishes sequence one"), Trace.BodyStageSequence, 1);
			TestEqual(TEXT("The exact requested body count is retained"), EffectiveBodySamples.Num(), ExpectedBodySamples.Num());
			TestTrue(
				TEXT("The exact requested body source is selected"),
				EffectiveBodySamples[0].Position.Equals(ExpectedBodySamples[0].Position, 0.0));
			TestEqual(
				TEXT("The effective fingerprint names the exact requested body source"),
				Trace.EffectiveBodyFingerprint,
				bUsesPriorBody ? Trace.PriorBodyFingerprint : Trace.LiveBodyFingerprint);

			float AppliedGroundHeightM = 0.0f;
			TestTrue(
				FString::Printf(TEXT("%s ground stage succeeds: %s"), GetFirstPolicyObservationSourceModeName(Mode), *Error),
				Trace.SelectGroundReference(
					...
					Error));
			TestEqual(TEXT("The ground stage publishes sequence two"), Trace.GroundStageSequence, 2);
			TestEqual(
				TEXT("The exact requested synthetic ground height is applied"),
				AppliedGroundHeightM,
				bUsesPriorGround
					? static_cast<float>(PriorGround.SyntheticGroundHeightM)
					: static_cast<float>(LiveGround.SyntheticGroundHeightM));
			...
			TestTrue(
				TEXT("A built self observation consumes the experiment exactly once"),
				Trace.MarkConsumed(PublishedFinalRootHeightM, Error));
			TestTrue(TEXT("The completed experiment validates"), ValidateFirstPolicyObservationSourceExperimentTrace(Trace, Error));
			TestEqual(TEXT("Configured state matches prior-body use"), Trace.bConfigured, bUsesPriorBody);
			TestTrue(TEXT("The requested mode was applied"), Trace.bApplied);
			TestTrue(TEXT("The applied source was consumed"), Trace.bConsumed);
			TestTrue(TEXT("The trace is complete"), Trace.bComplete);
			TestTrue(TEXT("The trace is valid"), Trace.bValid);
		};

		ExerciseMode(EPhysAnimFirstPolicyObservationSourceMode::LiveBodyLiveGround);
		ExerciseMode(EPhysAnimFirstPolicyObservationSourceMode::PriorBodyLiveGround);
		ExerciseMode(EPhysAnimFirstPolicyObservationSourceMode::PriorBodyPriorGround);

		EPhysAnimFirstPolicyObservationSourceMode ParsedMode =
			EPhysAnimFirstPolicyObservationSourceMode::LiveBodyLiveGround;
		FString Error;
		TestTrue(
			TEXT("The prior-body/live-ground command-line value parses"),
			TryParseFirstPolicyObservationSourceMode(TEXT("PriorBodyLiveGround"), ParsedMode, Error));
		TestEqual(
			TEXT("The prior-body/live-ground parser result is exact"),
			ParsedMode,
			EPhysAnimFirstPolicyObservationSourceMode::PriorBodyLiveGround);
		TestTrue(
			TEXT("The coherent prior command-line value parses"),
			TryParseFirstPolicyObservationSourceMode(TEXT("PriorBodyPriorGround"), ParsedMode, Error));
		TestFalse(
			TEXT("An unknown command-line source mode is rejected"),
			TryParseFirstPolicyObservationSourceMode(TEXT("Unknown"), ParsedMode, Error));
		TestFalse(TEXT("Invalid source mode rejection is explicit"), Error.IsEmpty());
		TestFalse(
			TEXT("The unchanged live-body/live-ground arm remains omission-only"),
			TryParseFirstPolicyObservationSourceMode(TEXT("LiveBodyLiveGround"), ParsedMode, Error));

		FPhysAnimFirstPolicyObservationSourceExperimentTrace MissingPriorTrace;
		TestTrue(
			TEXT("Missing-prior fixture configures prior body"),
			MissingPriorTrace.Configure(
				true,
				EPhysAnimFirstPolicyObservationSourceMode::PriorBodyLiveGround,
				Error));
		TArray<FPhysAnimBodySample> EffectiveBodySamples;
		TestFalse(
			TEXT("A configured prior-body mode rejects missing prior samples"),
			MissingPriorTrace.SelectBodySamples(
				TConstArrayView<FPhysAnimBodySample>(),
				LiveBodySamples,
				EffectiveBodySamples,
				Error));
		TestTrue(TEXT("Missing-prior rejection completes an invalid trace"), MissingPriorTrace.bComplete);
		TestFalse(TEXT("Missing-prior rejection cannot be valid"), MissingPriorTrace.bValid);

		FPhysAnimFirstPolicyObservationSourceExperimentTrace OrderingTrace;
		TestTrue(
			TEXT("Ordering fixture configures the default source"),
			OrderingTrace.Configure(
				false,
				EPhysAnimFirstPolicyObservationSourceMode::LiveBodyLiveGround,
				Error));
		float AppliedGroundHeightM = 0.0f;
		const FPhysAnimSelfObservationGroundReferenceValues LiveGround =
			MakeGroundReference(LiveBodySamples[0].Position.Z, 150.0);
		TestFalse(
			TEXT("Ground selection before body selection is rejected"),
			OrderingTrace.SelectGroundReference(
				&PriorGround,
				LiveGround,
				LiveBodySamples[0].Position.Z,
				AppliedGroundHeightM,
				Error));

		FPhysAnimFirstPolicyObservationSourceExperimentTrace OneShotTrace;
		TestTrue(
			TEXT("One-shot fixture configures the default source"),
			OneShotTrace.Configure(
				false,
				EPhysAnimFirstPolicyObservationSourceMode::LiveBodyLiveGround,
				Error));
		TestTrue(
			TEXT("One-shot fixture selects a body once"),
			OneShotTrace.SelectBodySamples(PriorBodySamples, LiveBodySamples, EffectiveBodySamples, Error));
		TestFalse(
			TEXT("The body stage cannot be repeated"),
			OneShotTrace.SelectBodySamples(PriorBodySamples, LiveBodySamples, EffectiveBodySamples, Error));

		FPhysAnimFirstPolicyObservationSourceExperimentTrace GroundOneShotTrace;
		TestTrue(
			TEXT("Ground one-shot fixture configures the default source"),
			GroundOneShotTrace.Configure(
				false,
				EPhysAnimFirstPolicyObservationSourceMode::LiveBodyLiveGround,
				Error));
		TestTrue(
			TEXT("Ground one-shot fixture selects its body"),
			GroundOneShotTrace.SelectBodySamples(PriorBodySamples, LiveBodySamples, EffectiveBodySamples, Error));
		TestTrue(
			TEXT("Ground one-shot fixture selects ground once"),
			GroundOneShotTrace.SelectGroundReference(
				&PriorGround,
				LiveGround,
				EffectiveBodySamples[0].Position.Z,
				AppliedGroundHeightM,
				Error));
		TestFalse(
			TEXT("The ground stage cannot be repeated"),
			GroundOneShotTrace.SelectGroundReference(
				&PriorGround,
				LiveGround,
				EffectiveBodySamples[0].Position.Z,
		...
		FPhysAnimFirstPolicyObservationSourceExperimentTrace RangeTrace;
		TestTrue(
			TEXT("Range fixture configures coherent prior source"),
			RangeTrace.Configure(
				true,
				EPhysAnimFirstPolicyObservationSourceMode::PriorBodyPriorGround,
				Error));
		TestTrue(
			TEXT("Range fixture selects the prior body"),
			RangeTrace.SelectBodySamples(PriorBodySamples, LiveBodySamples, EffectiveBodySamples, Error));
		FPhysAnimSelfObservationGroundReferenceValues InvalidPriorGround = PriorGround;
		InvalidPriorGround.CapsuleHalfHeightCm = -1.0;
		TestFalse(
			TEXT("The coherent ground stage rejects an invalid prior reference range"),
			RangeTrace.SelectGroundReference(
				&InvalidPriorGround,
				LiveGround,
				EffectiveBodySamples[0].Position.Z,
				AppliedGroundHeightM,
		...
		FPhysAnimFirstPolicyObservationSourceExperimentTrace NonFiniteTrace;
		TestTrue(
			TEXT("Non-finite fixture configures the default source"),
			NonFiniteTrace.Configure(
				false,
				EPhysAnimFirstPolicyObservationSourceMode::LiveBodyLiveGround,
				Error));
		TestTrue(
			TEXT("Non-finite fixture selects its body"),
			NonFiniteTrace.SelectBodySamples(PriorBodySamples, LiveBodySamples, EffectiveBodySamples, Error));
		TestFalse(
			TEXT("A non-finite effective root is rejected"),
			NonFiniteTrace.SelectGroundReference(
				&PriorGround,
				LiveGround,
				std::numeric_limits<double>::quiet_NaN(),
				AppliedGroundHeightM,
		...
		MissingPriorTrace.Reset();
		TestFalse(TEXT("Reset clears configured state"), MissingPriorTrace.bConfigured);
		TestFalse(TEXT("Reset clears body-stage state"), MissingPriorTrace.bBodyStageRecorded);
		TestFalse(TEXT("Reset clears ground-stage state"), MissingPriorTrace.bGroundStageRecorded);
		TestFalse(TEXT("Reset clears consumption state"), MissingPriorTrace.bConsumed);
		TestFalse(TEXT("Reset clears completion"), MissingPriorTrace.bComplete);
		TestTrue(TEXT("Reset clears errors"), MissingPriorTrace.Error.IsEmpty());
		return true;
	}
#endif
