...
namespace PhysAnimBridge
{
	...
	namespace
	{
		void FailFirstPolicyObservationSourceExperiment(
			FPhysAnimFirstPolicyObservationSourceExperimentTrace& Trace,
			const FString& Error)
		{
			if (Trace.Error.IsEmpty())
			{
				Trace.Error = Error;
			}
			Trace.bComplete = true;
			Trace.bValid = false;
		}

		bool ValidateFirstPolicyObservationSourceExperimentCore(
			const FPhysAnimFirstPolicyObservationSourceExperimentTrace& Trace,
			FString& OutError)
		{
			if (!Trace.Error.IsEmpty())
			{
				OutError = Trace.Error;
				return false;
			}
			if (!Trace.bComplete || !Trace.bApplied || !Trace.bConsumed ||
				!Trace.bBodyStageRecorded || !Trace.bGroundStageRecorded)
			{
				OutError = TEXT("First-policy observation-source experiment evidence is incomplete.");
				return false;
			}
			if (Trace.BodyStageSequence != 1 || Trace.GroundStageSequence != 2)
			{
				OutError = TEXT("Observation-source body and ground stages were not recorded in their locked order.");
				return false;
			}

			const bool bUsesPriorBody =
				Trace.RequestedMode != EPhysAnimFirstPolicyObservationSourceMode::LiveBodyLiveGround;
			if (Trace.bConfigured != bUsesPriorBody ||
				Trace.AppliedMode != Trace.RequestedMode ||
				Trace.RequestedModeName != GetFirstPolicyObservationSourceModeName(Trace.RequestedMode) ||
				Trace.AppliedModeName != Trace.RequestedModeName)
			{
				OutError = TEXT("Requested, configured, and applied observation-source modes disagree.");
				return false;
			}
			if (Trace.BodyFingerprintAlgorithm != FirstPolicyBodySourceFingerprintAlgorithm ||
				Trace.PriorBodyFingerprint.IsEmpty() ||
				Trace.LiveBodyFingerprint.IsEmpty() ||
				Trace.EffectiveBodyFingerprint.IsEmpty())
			{
				OutError = TEXT("Observation-source body fingerprint evidence is incomplete.");
				return false;
			}
			const FString& ExpectedEffectiveFingerprint = bUsesPriorBody
				? Trace.PriorBodyFingerprint
				: Trace.LiveBodyFingerprint;
			if (Trace.EffectiveBodyFingerprint != ExpectedEffectiveFingerprint)
			{
				OutError = TEXT("The effective body fingerprint does not match the requested body source.");
				return false;
			}

			const bool bFinite =
				FMath::IsFinite(Trace.PriorSyntheticGroundHeightM) &&
				FMath::IsFinite(Trace.LiveSyntheticGroundHeightM) &&
				FMath::IsFinite(Trace.AppliedSyntheticGroundHeightM) &&
				FMath::IsFinite(Trace.EffectiveBodyRootProtoZM) &&
				FMath::IsFinite(Trace.EffectiveFinalRootHeightM);
			if (!bFinite)
			{
				OutError = TEXT("Observation-source ground evidence contains a non-finite value.");
				return false;
			}

			const float ExpectedAppliedGround =
				Trace.RequestedMode == EPhysAnimFirstPolicyObservationSourceMode::PriorBodyPriorGround
					? static_cast<float>(Trace.PriorSyntheticGroundHeightM)
					: static_cast<float>(Trace.LiveSyntheticGroundHeightM);
			if (static_cast<float>(Trace.AppliedSyntheticGroundHeightM) != ExpectedAppliedGround)
			{
				OutError = TEXT("The applied synthetic ground height does not match the requested ground source.");
				return false;
			}
			const float ExpectedFinalRootHeightM = static_cast<float>(
				Trace.EffectiveBodyRootProtoZM - static_cast<double>(ExpectedAppliedGround));
			if (static_cast<float>(Trace.EffectiveFinalRootHeightM) != ExpectedFinalRootHeightM)
			{
				OutError = TEXT("The effective final root height does not match the applied float-path ground reference.");
				return false;
			}
			return true;
		}
	...
	const TCHAR* GetFirstPolicyObservationSourceModeName(
		const EPhysAnimFirstPolicyObservationSourceMode Mode)
	{
		switch (Mode)
		{
		case EPhysAnimFirstPolicyObservationSourceMode::LiveBodyLiveGround:
			return TEXT("LiveBodyLiveGround");
		case EPhysAnimFirstPolicyObservationSourceMode::PriorBodyLiveGround:
			return TEXT("PriorBodyLiveGround");
		case EPhysAnimFirstPolicyObservationSourceMode::PriorBodyPriorGround:
			return TEXT("PriorBodyPriorGround");
		default:
			return TEXT("Invalid");
		}
	}

	bool TryParseFirstPolicyObservationSourceMode(
		const FString& Value,
		EPhysAnimFirstPolicyObservationSourceMode& OutMode,
		FString& OutError)
	{
		OutMode = EPhysAnimFirstPolicyObservationSourceMode::LiveBodyLiveGround;
		OutError.Reset();
		if (Value == TEXT("PriorBodyLiveGround"))
		{
			OutMode = EPhysAnimFirstPolicyObservationSourceMode::PriorBodyLiveGround;
			return true;
		}
		if (Value == TEXT("PriorBodyPriorGround"))
		{
			OutMode = EPhysAnimFirstPolicyObservationSourceMode::PriorBodyPriorGround;
			return true;
		}
		OutError = FString::Printf(
			TEXT("Unsupported PhysAnimFirstPolicyObservationSourceExperiment value '%s'. Expected PriorBodyLiveGround or PriorBodyPriorGround."),
			*Value);
		return false;
	}

	bool FPhysAnimFirstPolicyObservationSourceExperimentTrace::Configure(
		const bool bInConfigured,
		const EPhysAnimFirstPolicyObservationSourceMode InRequestedMode,
		FString& OutError)
	{
		Reset();
		OutError.Reset();
		...
		RequestedModeName = GetFirstPolicyObservationSourceModeName(InRequestedMode);
		const bool bValidConfiguration = bInConfigured
			? InRequestedMode != EPhysAnimFirstPolicyObservationSourceMode::LiveBodyLiveGround
			: InRequestedMode == EPhysAnimFirstPolicyObservationSourceMode::LiveBodyLiveGround;
		if (!bValidConfiguration || RequestedModeName == TEXT("Invalid"))
		{
			OutError = TEXT("The default observation source must be omitted, while prior-source modes must be explicit.");
			FailFirstPolicyObservationSourceExperiment(*this, OutError);
			return false;
		}
		return true;
	}

	void FPhysAnimFirstPolicyObservationSourceExperimentTrace::RejectConfiguration(
		const FString& InRequestedModeName,
		const FString& InError)
	{
		Reset();
		bConfigured = true;
		RequestedModeName = InRequestedModeName;
		FailFirstPolicyObservationSourceExperiment(
			*this,
			InError.IsEmpty() ? TEXT("The observation-source experiment configuration was invalid.") : InError);
	}

	bool FPhysAnimFirstPolicyObservationSourceExperimentTrace::SelectBodySamples(
		TConstArrayView<FPhysAnimBodySample> PriorBodySamples,
		TConstArrayView<FPhysAnimBodySample> LiveBodySamples,
		TArray<FPhysAnimBodySample>& OutEffectiveBodySamples,
		FString& OutError)
	{
		OutError.Reset();
		OutEffectiveBodySamples.Reset();
		if (!Error.IsEmpty())
		{
			OutError = Error;
			return false;
		}
		if (bBodyStageRecorded || bGroundStageRecorded || bConsumed)
		{
			OutError = TEXT("The first-policy observation-source body stage is one-shot.");
			FailFirstPolicyObservationSourceExperiment(*this, OutError);
			return false;
		}

		if (!ValidateFirstPolicyBodySamples(LiveBodySamples, TEXT("Live first-policy body source"), OutError) ||
			!BuildFirstPolicyBodySourceFingerprint(LiveBodySamples, LiveBodyFingerprint, OutError))
		{
			FailFirstPolicyObservationSourceExperiment(*this, OutError);
			return false;
		}

		FString PriorValidationError;
		const bool bPriorValid =
			ValidateFirstPolicyBodySamples(PriorBodySamples, TEXT("Prior first-policy body source"), PriorValidationError) &&
			BuildFirstPolicyBodySourceFingerprint(PriorBodySamples, PriorBodyFingerprint, PriorValidationError);
		const bool bUsesPriorBody =
			RequestedMode != EPhysAnimFirstPolicyObservationSourceMode::LiveBodyLiveGround;
		if (bUsesPriorBody && !bPriorValid)
		{
			OutError = FString::Printf(
				TEXT("The configured observation-source experiment requires a valid prior body source: %s"),
				*PriorValidationError);
			FailFirstPolicyObservationSourceExperiment(*this, OutError);
			return false;
		}

		const TConstArrayView<FPhysAnimBodySample> EffectiveBodySamples = bUsesPriorBody
			? PriorBodySamples
			: LiveBodySamples;
		OutEffectiveBodySamples.Append(EffectiveBodySamples.GetData(), EffectiveBodySamples.Num());
		if (!BuildFirstPolicyBodySourceFingerprint(
				OutEffectiveBodySamples,
		...
		BodyFingerprintAlgorithm = FirstPolicyBodySourceFingerprintAlgorithm;
		bBodyStageRecorded = true;
		BodyStageSequence = 1;
		return true;
	}

	bool FPhysAnimFirstPolicyObservationSourceExperimentTrace::SelectGroundReference(
		const FPhysAnimSelfObservationGroundReferenceValues* PriorGroundReference,
		const FPhysAnimSelfObservationGroundReferenceValues& LiveGroundReference,
		const double InEffectiveBodyRootProtoZM,
		float& OutAppliedSyntheticGroundHeightM,
		FString& OutError)
	{
		OutError.Reset();
		OutAppliedSyntheticGroundHeightM = static_cast<float>(LiveGroundReference.SyntheticGroundHeightM);
		if (!Error.IsEmpty())
		{
			OutError = Error;
			return false;
		...
		if (!bBodyStageRecorded || bGroundStageRecorded || bConsumed)
		{
			OutError = TEXT("The first-policy observation-source ground stage must run once after the body stage.");
			FailFirstPolicyObservationSourceExperiment(*this, OutError);
			return false;
		}
		if (!ValidateGroundReferenceValues(LiveGroundReference, OutError) ||
			!FMath::IsFinite(InEffectiveBodyRootProtoZM))
		{
			if (OutError.IsEmpty())
			{
				OutError = TEXT("The effective first-policy body root is non-finite.");
			}
			FailFirstPolicyObservationSourceExperiment(*this, OutError);
			return false;
		}

		LiveSyntheticGroundHeightM = LiveGroundReference.SyntheticGroundHeightM;
		if (PriorGroundReference)
		{
			FString PriorGroundError;
			if (ValidateGroundReferenceValues(*PriorGroundReference, PriorGroundError))
			{
				PriorSyntheticGroundHeightM = PriorGroundReference->SyntheticGroundHeightM;
			}
			else if (RequestedMode == EPhysAnimFirstPolicyObservationSourceMode::PriorBodyPriorGround)
			{
				OutError = FString::Printf(
					TEXT("The coherent prior observation source requires a valid prior ground reference: %s"),
					*PriorGroundError);
				FailFirstPolicyObservationSourceExperiment(*this, OutError);
				return false;
			}
		...
		{
			OutError = TEXT("The coherent prior observation source requires a captured prior ground reference.");
			FailFirstPolicyObservationSourceExperiment(*this, OutError);
			return false;
		}

		OutAppliedSyntheticGroundHeightM =
			RequestedMode == EPhysAnimFirstPolicyObservationSourceMode::PriorBodyPriorGround
				? static_cast<float>(PriorSyntheticGroundHeightM)
				: static_cast<float>(LiveSyntheticGroundHeightM);
		AppliedSyntheticGroundHeightM = OutAppliedSyntheticGroundHeightM;
		EffectiveBodyRootProtoZM = InEffectiveBodyRootProtoZM;
		EffectiveFinalRootHeightM = static_cast<float>(
			InEffectiveBodyRootProtoZM - static_cast<double>(OutAppliedSyntheticGroundHeightM));
		AppliedMode = RequestedMode;
		AppliedModeName = GetFirstPolicyObservationSourceModeName(AppliedMode);
		bApplied = true;
		bGroundStageRecorded = true;
		GroundStageSequence = 2;
		return true;
	}

	bool FPhysAnimFirstPolicyObservationSourceExperimentTrace::MarkConsumed(
		const float PublishedFinalRootHeightM,
		FString& OutError)
	{
		OutError.Reset();
		if (!Error.IsEmpty())
		...
		if (!bGroundStageRecorded || bConsumed)
		{
			OutError = TEXT("The first-policy observation-source experiment can be consumed exactly once after ground selection.");
			FailFirstPolicyObservationSourceExperiment(*this, OutError);
			return false;
		}
		EffectiveFinalRootHeightM = PublishedFinalRootHeightM;
		bConsumed = true;
		bComplete = true;
		bValid = ValidateFirstPolicyObservationSourceExperimentCore(*this, OutError);
		if (!bValid)
		{
			FailFirstPolicyObservationSourceExperiment(*this, OutError);
		}
	...
	void FPhysAnimFirstPolicyObservationSourceExperimentTrace::Reset()
	{
		*this = FPhysAnimFirstPolicyObservationSourceExperimentTrace();
	}

	bool ValidateFirstPolicyObservationSourceExperimentTrace(
		const FPhysAnimFirstPolicyObservationSourceExperimentTrace& Trace,
		FString& OutError)
	{
		OutError.Reset();
		if (!ValidateFirstPolicyObservationSourceExperimentCore(Trace, OutError))
		{
			return false;
		}
		if (!Trace.bValid)
		{
			OutError = TEXT("A complete observation-source experiment did not publish a valid verdict.");
			return false;
		}
		return true;
	}
