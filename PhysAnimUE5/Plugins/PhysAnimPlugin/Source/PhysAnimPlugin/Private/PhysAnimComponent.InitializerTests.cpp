...
namespace
{
	...
	bool FPhysAnimStage1InitializerDefaultsTest::RunTest(const FString& Parameters)
	{
		...
		for (TPair<FName, FInitialPhysicsControl>& Pair : Initializer->InitialControls)
		{
			...
			TestEqual(*FString::Printf(TEXT("%s uses solver-stable extra damping"), *Pair.Key.ToString()), Pair.Value.ControlData.AngularExtraDamping, 0.5f);
		...
		if (MutatedControl)
		{
			...
			TestEqual(TEXT("Runtime defaults replace stale serialized extra damping"), MutatedControl->ControlData.AngularExtraDamping, 0.5f);
		}
		return true;
	}
