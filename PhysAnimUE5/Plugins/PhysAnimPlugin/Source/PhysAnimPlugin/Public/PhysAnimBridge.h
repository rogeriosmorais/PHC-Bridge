...
#if WITH_DEV_AUTOMATION_TESTS
	enum class EPhysAnimFirstPolicyObservationSourceMode : uint8
	{
		LiveBodyLiveGround,
		PriorBodyLiveGround,
		PriorBodyPriorGround
	};

	PHYSANIMPLUGIN_API const TCHAR* GetFirstPolicyObservationSourceModeName(
		EPhysAnimFirstPolicyObservationSourceMode Mode);

	PHYSANIMPLUGIN_API bool TryParseFirstPolicyObservationSourceMode(
		const FString& Value,
		EPhysAnimFirstPolicyObservationSourceMode& OutMode,
		FString& OutError);

	struct PHYSANIMPLUGIN_API FPhysAnimFirstPolicyObservationSourceExperimentTrace
	{
		bool Configure(
			bool bInConfigured,
			EPhysAnimFirstPolicyObservationSourceMode InRequestedMode,
			FString& OutError);

		void RejectConfiguration(const FString& InRequestedModeName, const FString& InError);

		bool SelectBodySamples(
		...
		bool SelectGroundReference(
			...
			double EffectiveBodyRootProtoZM,
			float& OutAppliedSyntheticGroundHeightM,
			FString& OutError);

		bool MarkConsumed(float PublishedFinalRootHeightM, FString& OutError);
		void Reset();

		bool bConfigured = false;
		EPhysAnimFirstPolicyObservationSourceMode RequestedMode =
			EPhysAnimFirstPolicyObservationSourceMode::LiveBodyLiveGround;
		FString RequestedModeName;
		EPhysAnimFirstPolicyObservationSourceMode AppliedMode =
			EPhysAnimFirstPolicyObservationSourceMode::LiveBodyLiveGround;
		FString AppliedModeName;
		bool bApplied = false;
		bool bConsumed = false;
		bool bBodyStageRecorded = false;
		int32 BodyStageSequence = INDEX_NONE;
		bool bGroundStageRecorded = false;
		int32 GroundStageSequence = INDEX_NONE;
		FString BodyFingerprintAlgorithm;
		FString PriorBodyFingerprint;
		FString LiveBodyFingerprint;
		FString EffectiveBodyFingerprint;
		double PriorSyntheticGroundHeightM = 0.0;
		double LiveSyntheticGroundHeightM = 0.0;
		double AppliedSyntheticGroundHeightM = 0.0;
		double EffectiveBodyRootProtoZM = 0.0;
		double EffectiveFinalRootHeightM = 0.0;
		bool bComplete = false;
		bool bValid = false;
		FString Error;
	};

	PHYSANIMPLUGIN_API bool ValidateFirstPolicyObservationSourceExperimentTrace(
