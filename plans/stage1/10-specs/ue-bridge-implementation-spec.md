...
It exists because [bridge-spec.md](/F:/NewEngine/plans/stage1/10-specs/bridge-spec.md) locks the model-facing contract, but does not by itself freeze the exact UE `5.7.3` classes, functions, ownership model, content paths, and tick/update flow needed to build the runtime safely.
...
- G1 smoke checks already defined in [g1-evidence.md](/F:/NewEngine/plans/stage1/30-evidence/g1-evidence.md)
- G2 side-by-side comparison defined in [comparison-sequence-lock.md](/F:/NewEngine/plans/stage1/50-content/comparison-sequence-lock.md)
