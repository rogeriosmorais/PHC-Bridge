...
| A-10 | `PhysAnim.PIE.BalanceModeSmoke` can report `BalanceActive_Standing` while metrics show `PelvisSim=0`. | Blocked | A balance proof needs live physical continuity evidence for the pelvis/truth set. | Do not approve G2 from the current smoke pass until this contradiction is audited or fixed. | Physical-continuity truth audit task. |
