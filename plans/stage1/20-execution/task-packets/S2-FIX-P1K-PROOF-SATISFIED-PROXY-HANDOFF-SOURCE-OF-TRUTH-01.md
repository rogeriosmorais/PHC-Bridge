...
## Required Work
1. Supersede the blocked proxy-timing portion of `S2-FIX-P1J-ACTIVATION-PATH-PROXY-TIMING-SURFACE-WIDEN-01`.
2. Fix only this current symptom: `PhysAnim.ActivationPath.Wiring.ProofSatisfied` fails during proof-satisfied activation with `ENTRY_DENIED reason=PROXY_OUTSIDE_HULL`.
...
4. Do not work on proof-disabled safe-deny path, proof gate truth criteria, support truth generation, validators, runtime adapter, runtime orchestrator, termination pipeline, failure arbitration, locomotion chain, or tuning.
...
7. In `PhysAnimRuntimeTerminationState.*` and `PhysAnimRuntimeTermination.*`:
   - add or refine only narrow state needed to distinguish recorded startup proxy terminal, deferred startup proxy terminal, and enforceable post-handoff proxy terminal
...
9. In the activation wiring tests:
   - `ProofSatisfied` must assert fresh proof completion, standing-entry attempt, no denial by startup-deferred `PROXY_OUTSIDE_HULL`, and the expected activation/standing path
...
10. Add or extend regression coverage for:
