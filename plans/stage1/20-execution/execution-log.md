...
- `Latest runtime forensics`: the latest saved verified live smoke on `2026-04-22` still clears Phase 1 admission and Phase 2 RootOn, reaches `BalanceEntry_Settle`, and preserves the explicit transition-owned shell lock into Phase 3. It then fails before sustained standing and therefore fails the benchmark. The current audited blocker is `PHASE3_FIRST_FAILURE_AUDIT frame=943`, `tick=2`, `phase3_material_shell_correction`, with `rootLinear=49.13/3000.00`, `rootAngular=190.06/2160.00`, `shellVelocityDelta=33.21/10.00`, `currentMaxNonRootAngular=1506.07` on `spine_01`, and `currentNonRootFamilyAngular=3378.41`.
- `Tracked evidence sync`: the checked-in `test-results/phase1-autocalib/automation_phase1_smoke` artifacts were refreshed again after the stale standing-hold reset fix. They still match the `BalanceActive_Standing` `3.0`-second success contract, still report no passing candidate and no best candidate, and still show `phase3_material_shell_correction` as the dominant truthful blocker, with one isolated `phase2_root_on_spike` elsewhere in the blocker counts.
