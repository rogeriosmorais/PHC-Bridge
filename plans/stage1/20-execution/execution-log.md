...
- `Latest runtime forensics`: the latest saved verified live smoke on `2026-04-22` still clears Phase 1 admission and Phase 2 RootOn, reaches `BalanceEntry_Settle`, and preserves the explicit transition-owned shell lock into Phase 3. It then fails before sustained standing and therefore fails the benchmark. The current audited blocker is `PHASE3_FIRST_FAILURE_AUDIT frame=943`, `tick=2`, `phase3_material_shell_correction`, with `rootLinear=49.12/3000.00`, `rootAngular=190.07/2160.00`, `shellVelocityDelta=33.20/10.00`, `currentMaxNonRootAngular=1506.08` on `spine_01`, and `currentNonRootFamilyAngular=3378.43`.
- `Tracked evidence sync`: the checked-in `test-results/phase1-autocalib/automation_phase1_smoke` artifacts were refreshed after the standing-benchmark pivot. They now match the `BalanceActive_Standing` `3.0`-second success contract and currently report no passing candidate, no best candidate, and a dominant truthful blocker of `phase3_material_shell_correction`.
